...
import { LinkExternal, Text } from '@crystals/uikit'
...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({
  ...
  const { dual } = farm
  const lpLabel = farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('CRYSTL', '')
  const lpAddress = farm.lpAddresses[process.env.REACT_APP_CHAIN_ID]
  const bsc = getBscScanAddressUrl(lpAddress)
  const info = getTokenInfoUrl(lpAddress)
  ...
  return (
    <Container expanded={expanded}>
      <InfoContainer>
        {isActive && (
          <StakeContainer>
            <StyledLinkExternal href={getToken}>{t('Get %symbol%', { symbol: lpLabel })}</StyledLinkExternal>
          </StakeContainer>
        )}
        <StyledLinkExternal href={projectLink}>{t('View Project Site')}</StyledLinkExternal>
        <StyledLinkExternal href={bsc}>{t('View Contract')}</StyledLinkExternal>
        <StyledLinkExternal href={info}>{t('View Info Site')}</StyledLinkExternal>
        <TagsContainer>
