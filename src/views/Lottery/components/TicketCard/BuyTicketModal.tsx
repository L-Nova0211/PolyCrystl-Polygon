...
import { Button, Modal } from '@crystals/uikit'
...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  const crystlCosts = (amount: string): number => {
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        availableSymbol="CRYSTL"
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% CRYSTL', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your CRYSTL cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% CRYSTL', { num: crystlCosts(val) })}</Final>
      </div>
      <ModalActions>
