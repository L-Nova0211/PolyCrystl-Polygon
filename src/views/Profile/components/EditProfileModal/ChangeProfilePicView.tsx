...
import { Button, InjectedModalProps, Skeleton, Text } from '@crystals/uikit'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onApprove: () => {
        return contract.methods.approve(getPolyCrystalProfileAddress(), selectedNft.tokenId).send({ from: account })
      },
      onConfirm: () => {
        if (!profile.isActive) {
    ...
    <>
      ...
      {!isLoading && nftsInWallet.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {t('Make sure you have a PolyCrystal Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
