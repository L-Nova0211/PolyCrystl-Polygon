...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@crystals/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberCrystlToReactivate } = useGetProfileCosts()
  const { t } = useTranslation()
  const polyCrystalProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = () => {
    polyCrystalProfileContract.methods
      .pauseProfile()
      .send({ from: account })
      .on('sending', () => {
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t('Cost to reactivate in the future: %cost% CRYSTL', { cost: getBalanceNumber(numberCrystlToReactivate) })}
      </Text>
