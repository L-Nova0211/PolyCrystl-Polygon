...
import { Button, useModal, IconButton, AddIcon, MinusIcon, Skeleton, useTooltip, Flex, Text } from '@crystals/uikit'
...
import VaultStakeModal from '../../CrystlVaultCard/VaultStakeModal'
...
const Staked: React.FunctionComponent<StackedActionProps> = ({ pool, userDataLoaded }) => {
  ...
  const stakedAutoDollarValue = getBalanceNumber(
    crystlAsBigNumber.multipliedBy(stakingTokenPrice),
    stakingToken.decimals,
  )

  const needsApproval = isAutoVault ? !isVaultApproved : !allowance.gt(0) && !isBnbPool

  const displayDecimals = stakedTokenBalance < 0.00001 ? 8 : 5
  ...
  const [onPresentVaultUnstake] = useModal(
    <VaultStakeModal stakingMax={crystlAsBigNumber} pool={pool} isRemovingStake />,
  )
        ...
        <ActionContent>
          <Flex flex="1" pt="16px" flexDirection="column" alignSelf="flex-start">
            <Balance
              ...
              value={isAutoVault ? crystlAsNumberBalance : stakedTokenBalance}
            />
            <Balance
              fontSize="12px"
              display="inline"
